"""S15-N10 * apply the composed stages ONLY where they do not touch the predictive density.

S15-N9 ported all four surviving stages into member D and the +0.003623 that they were worth on
the POINT pipeline reversed to -0.002546 against the honest champion baseline: the composed
member's solo fell to 0.6227-0.6246 against the original D's 0.625669.

The cause is identifiable and it is now the seventh independent replication of one law in this
project.  D1 is extra_trees + path_smooth + feature_fraction_bynode, and all three are
SMOOTHING regularisers.  Applied to the physics teacher, where the output is a point estimate,
they were worth +0.002685.  Applied to the 26-class DART classifier, they flatten exactly the
distributional sharpness the settlement argmax feeds on.  The same boundary destroyed probability
averaging (S12-N1), valid-conditional training (S12-N15), distribution recentring (S12-N16),
metric-matched weights inside the classifier (S13-N5), self-training (S14-N12) and seed averaging
of the member (S15-N4).

THE LAW, stated so it can be designed around rather than rediscovered: any operation that raises
point accuracy by smoothing the predictive distribution loses more FICR than it gains NMAE,
because the decision layer's value comes from sharpness, not from the conditional mean.

TREATMENT: keep B1, B2 and A3 -- which act on the FEATURES and the TEACHER TARGET, upstream of the
density -- and apply D1 to the teacher and the hub-wind model ONLY, leaving the DART classifier
byte-identical to the champion's.
"""
_ORIGINAL_DOC = """S15-N9 * port the surviving stages into member D and run the full champion pipeline.
...
if __name__ == '__main__':
    ...
    for f, (a, b) in FOLDS.items():
        ...
        for sd in SEEDS:
            ...
            dp = dict(DART); dp['random_state'] = sd          # D1 NOT applied to the density
    ...
    for s, P in Ps.items():
        np.save(N + f'S15-N10_prob_{s}.npy', P)
    ...
    json.dump({'per_seed': tot.tolist(), 'mean': float(tot.mean()), 'sd': float(tot.std(ddof=1)),
              ...
              open(N + 'S15-N10_member_nodensity.json', 'w'), indent=1, default=str)
