...
"""S9-N6 fold-outside gate for the REWS (terrain candidate E) screening result. Mirrors
s9_n4/n5_foldoutside.py -- rews_features() is also purely deterministic (two-point
log-law solve on domain-mean wind, nothing fit from data), so no per-fold refit of the
feature itself is needed.
"""
import sys, json
...
if __name__ == '__main__':
    ...
    A_r = pd.concat(FR.values())
    COLS_r = COLS0 + list(feats.columns)
    r, r_picks = score_variant('rews', A_r, COLS_r)
    print(f'REWS(E) fold-outside: {r["total"]:.6f} picks={r_picks}', flush=True)

    delta = r['total'] - ctrl['total']
    print(f'DELTA (fold-outside): {delta:+.6f}', flush=True)
    json.dump({'control': ctrl, 'control_picks': ctrl_picks,
               'rews_e': r, 'rews_e_picks': r_picks, 'delta_foldoutside': delta},
              open('/Users/um-yunsang/BARAM2026/research/nodes/S9-N6_foldoutside.json', 'w'),
